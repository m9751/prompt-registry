...
| `PRM-INFRA-008` | [Memory Drift Loop — Iterate the Memory Store to Drift-Clear](prompts/systems-architecture/PRM-INFRA-008_memory-drift-loop.md) | Iterate a memory store to drift-clear by driving the PRM-INFRA-005 audit in a one-approved-change-per-iteration loop against the three drift counts, with a human gate on every write and a re-audit that confirms each finding cleared, until drift-clear or an exhausted budget | PRM-INFRA-005 Phase-1 drift lists (structured counts) + operator approvals | Claude Code (loop driver) + PRM-INFRA-005 as the read-only drift signal | `1.0.0` | [View File](prompts/systems-architecture/PRM-INFRA-008_memory-drift-loop.md) |

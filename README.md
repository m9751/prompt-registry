...
| `PRM-INFRA-007` | [Repo Hardening Loop — Iterate a Repo to Agent-Execution-Ready](prompts/systems-architecture/PRM-INFRA-007_repo-hardening-loop.md) | Iterate a repo to agent-execution-ready by driving the PRM-CDXP-002 audit in a one-change-per-iteration loop against AERR + M9 until a resolved bar or an exhausted budget | Git repository (filesystem) + PRM-CDXP-002 audit snapshots (§13 JSON) + M9 paired-task results | Claude Code (loop driver, writable) + Codex exec read-only (audit signal) | `1.0.2` | [View File](prompts/systems-architecture/PRM-INFRA-007_repo-hardening-loop.md) |
| `PRM-INFRA-008` | [Memory Drift Loop — Iterate the Memory Store to Drift-Clear](prompts/systems-architecture/PRM-INFRA-008_memory-drift-loop.md) | Iterate a memory store to drift-clear by driving the PRM-INFRA-005 audit in a one-approved-change-per-iteration loop against the three drift counts, with a human gate on every write and a re-audit that confirms each finding cleared, until drift-clear or an exhausted budget | PRM-INFRA-005 Phase-1 drift lists (structured counts) + operator approvals | Claude Code (loop driver) + PRM-INFRA-005 as the read-only drift signal | `1.0.1` | [View File](prompts/systems-architecture/PRM-INFRA-008_memory-drift-loop.md) |

...
- {Account}-Slides-YYYY-MM-DD.html — browser-viewable slide deck, 1280x720px per slide
- {Account}-Slides-Code-YYYY-MM-DD.gs — paste into script.google.com and Run to create the deck in Google Drive

Trigger phrase: "build me an HTML slide-ready presentation"

Division of labor: Claude generates the .gs (structured, deterministic, no rendering needed). For visual polish on the HTML, hand it to Gemini (free, no token cost).
...
- 6-DIGIT HEX ONLY — never rgba(), never 8-digit hex (e.g. #FF000080 is forbidden)
- CSS variables in :root — use brand block values
- Bottom accent bar on every slide: position: absolute; bottom: 0; left: 0; width: 1280px; height: 4px; background: var(--accent)

Slide type templates:

TYPE: title — navy background
...
- Bottom half: layered architecture diagram — 4 horizontal boxes (Experience, Process API, System API, Source System), each labeled and colored by layer, connected by arrow symbols

TYPE: steps — white background
...
TYPE: table — white background
- Navy top bar: title
- Table with header row (cloud bg) and body rows: columns are #, Requirement (name + description), What We Built, How We Tested It, Result pill
- Result pill: amber bg + amber text for BUILT/PENDING; green bg + green text for PASS

TYPE: next-steps — white background
- Navy top bar: title + subtitle
...
1. 6-digit hex only — scan every setSolidFill() call before outputting. If any hex value matches the pattern #[0-9A-Fa-f]{8}, replace with nearest 6-digit equivalent. Hard stop: never ship 8-digit hex.
...
7. Slide dimensions — use points. 720x405pt maps to 1280x720px at 96dpi. Use var W = 720; var H = 405.

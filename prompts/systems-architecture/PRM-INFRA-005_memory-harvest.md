...
2. Flag STALE-SUSPECT entries: any entry naming a file, flag, or PR — spot-check that the artifact still exists (a memory reflects what was true when written, not necessarily now). Check the artifact in the repo it actually belongs to, not the current working directory: MEMORY.md and its cited PRs live in claude-config, not wherever the session happens to be running. A `gh pr view <n>` that returns not-found from the wrong repo is INCONCLUSIVE, never confirmed-stale — resolve the correct repo (`gh pr view <n> --repo m9751/claude-config`, or smokin-os for a build PR) before flagging. A STALE flag on an artifact that in fact exists is a false positive that inflates the drift count.
...
6. After the three lists, emit a structured count block so a drift-reduction loop (e.g. PRM-INFRA-008) can diff drift across iterations without re-parsing the prose. Emit it as a fenced JSON block whose info-string is the single token `infra005-drift-counts` (a json fence tagged with that label), containing exactly these integer fields: `contradictions_internal`, `contradictions_crossstore`, `stale_suspect`, `rule_candidates_promote`, `rule_candidates_landed`, and `total_open_drift` = contradictions_internal + contradictions_crossstore + stale_suspect + rule_candidates_promote (rule_candidates_landed is already enforced and is NOT open drift, so exclude it). The counts must match the three lists exactly. Count a CROSS-STORE contradiction by the number of DISTINCT live Hindsight facts still asserting the superseded claim, not by the number of topics: if one MEMORY.md correction is contradicted by four duplicate live facts, that is 4 open cross-store items, not 1 — because closing it takes four invalidates (one per fact), and a drift-reduction loop that invalidates one and re-audits must see the count only partially drop. A single topic backed by N duplicate stale facts is N in this count. (Derived observations regenerate from their source facts and cannot be invalidated directly — do not count an observation as a separate open item; it clears when its sources do.)
...
- For every STALE-SUSPECT PR/file, did I check it in the repo it belongs to (claude-config for MEMORY.md and its PRs; smokin-os for build PRs) and NOT flag on a not-found from the wrong working directory?
- Did I count each CROSS-STORE contradiction by the number of distinct live facts asserting it (so a claim duplicated across N facts is N, not 1), and exclude derived observations from the count?
```
